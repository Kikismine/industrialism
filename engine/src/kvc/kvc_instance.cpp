...
KvcInstance::KvcInstance(std::string _appName, std::vector<std::uint8_t> _appVersion, std::string _engineName, std::vector<std::uint8_t> _engineVersion) : appName{std::move(_appName)}, engineName{std::move(_engineName)}, appVersion{std::move(_appVersion)}, engineVersion{std::move(_engineVersion)} {
    createInstance();
    setupDebugMessenger();
}
...
std::vector<const char*> KvcInstance::getRequiredExtensions() const {
